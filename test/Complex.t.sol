...
contract ComplexTest is Test {
    ...
    function testAddZ() public {
        (int256 r, int256 i) = complex.addZ(int256(1) * scale, int256(2) * scale, int256(3) * scale, int256(4) * scale);
        assertEq(r / scale, 3);
        assertEq(i / scale, 7);
    }

    function testSubZ() public {
        (int256 r, int256 i) = complex.subZ(3 * scale, 2 * scale, 5 * scale, 1 * scale);
        assertEq(r / scale, 1);
        assertEq(i / scale, 4);
    }

    function testMulZ() public {
        (int256 r, int256 i) = complex.addZ(1 * scale, 3 * scale, 1 * scale, 2 * scale);
        assertEq(r / scale, 1);
        assertEq(i / scale, 5);
    }
    ...
    function testCalcR() public {
        uint256 r = complex.calcR(3 * scale, 4 * scale);
        assertEq(r / uint256(scale), 5);
    }

    function testToPolar() public {
        (int256 r, int256 t) = complex.toPolar(3 * scale, 4 * scale);
